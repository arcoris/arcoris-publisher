...
run_step "resetting lab for candidate failure"
bash "$LAB_DIR/setup.sh"
write_reject_hook "$(bare_repo arcoris/control)" candidate-control

report="$(lab_reports)/candidate-failure.json"
stderr="$(lab_logs)/candidate-failure.stderr"

run_step "running publish with candidate ref rejection"
...
log "candidate failure rollback demonstrated: $tx_id"
