...
run_step "resetting lab for tag failure"
bash "$LAB_DIR/setup.sh"
write_reject_hook "$(bare_repo arcoris/foundation)" tags

report="$(lab_reports)/tag-failure.json"
stderr="$(lab_logs)/tag-failure.stderr"

run_step "running publish with tag rejection"
...
log "tag failure rollback demonstrated: $tx_id"
