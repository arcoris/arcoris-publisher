...
// Publish builds a plan, verifies targets, and publishes them when checks pass.
func (a App) Publish(ctx context.Context, req Request) (Result, error) {
	plan, err := a.BuildPlan(ctx, req.ManifestPath, req.Version)
	if err != nil {
		return Result{}, err
	}

	result, err := workflow.New(a.workflowDeps, a.workflowOptions).Run(ctx, workflow.Request{
		Plan:                plan,
