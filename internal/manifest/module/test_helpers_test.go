...
func validSpec() modulemanifest.Spec {
	return modulemanifest.Spec{
		APIVersion:   string(manifest.APIVersionV1Alpha1),
		Kind:         string(manifest.KindModuleManifest),
		Metadata:     manifest.MetadataSpec{Name: "control"},
		Module:       manifest.ModuleIdentitySpec{Path: "arcoris.dev/control"},
		Dependencies: modulemanifest.DependenciesSpec{Internal: []string{"foundation"}},
