...
func TestParsePublishEntryKindAcceptsKnownKinds(t *testing.T) {
	for _, want := range []manifest.PublishEntryKind{manifest.PublishEntryFile, manifest.PublishEntryDirectory} {
		...
		if got != want {
			t.Fatalf("ParsePublishEntryKind(%q) = %q", want, got)
		}
	}
}

func TestParsePublishEntryKindRejectsUnknownKind(t *testing.T) {
	if _, err := manifest.ParsePublishEntryKind("symlink"); err == nil {
		t.Fatalf("expected unsupported publish entry kind error")
	}
}
