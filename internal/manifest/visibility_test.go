...
func TestParseVisibilityAcceptsKnownValues(t *testing.T) {
	for _, want := range []manifest.Visibility{manifest.VisibilityPublic, manifest.VisibilityInternal, manifest.VisibilityDisabled} {
		...
		if got != want || got.String() != string(want) {
			t.Fatalf("ParseVisibility(%q) = %q", want, got)
		}
	}
}

func TestParseVisibilityRejectsUnknownValue(t *testing.T) {
	if _, err := manifest.ParseVisibility("private"); err == nil {
