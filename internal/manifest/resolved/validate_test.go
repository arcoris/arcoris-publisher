...
func TestResolveRejectsPublicDependencyOnInternal(t *testing.T) {
	...
	_, err := resolved.Resolve(resolved.ResolveInput{Staging: stagingManifest(t, spec), Modules: standardModules(t)})
	if err == nil {
...
func TestResolveRejectsDuplicatePublicRepositories(t *testing.T) {
	...
	_, err := resolved.Resolve(resolved.ResolveInput{Staging: stagingManifest(t, spec), Modules: []modulemanifest.Manifest{
		moduleManifest(t, "foundation", "arcoris.dev/foundation", nil),
