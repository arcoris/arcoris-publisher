...
func TestResolveRejectsDuplicateModuleManifestNames(t *testing.T) {
	_, err := resolved.Resolve(resolved.ResolveInput{
		...
		Modules: []modulemanifest.Manifest{
			...
			moduleManifest(t, "foundation", "arcoris.dev/foundation-copy", nil),
		},
	})
	if err == nil {
		t.Fatalf("expected duplicate module manifest error")
	}
}
